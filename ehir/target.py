...
# def process_align_camera(img, tpl_distmap, tpl_img, init_bbox, **kws):   # infer
def process_align_camera(img, tpl_distmap, init_bbox, **kws):              # test_DeepPCB
    src_img = get_gray(img)
    tpl_w = tpl_distmap.shape[1]

    x0, y0, x1, y1 = init_bbox
    moving_img = src_img[y0:y1, x0:x1]
    # moving_img = src_img      # test for draw 

    ret = align_edge(
        moving_img,
        {'distmap': tpl_distmap,},
        # tpl_img, ### add for test infer
        init_bbox=init_bbox,
        **kws,
    )
